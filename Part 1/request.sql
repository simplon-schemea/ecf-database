...
SELECT matricule, lastname, firstname, tech.name_fr as main_tech_fr
FROM developers dev
...
SELECT matricule, lastname, firstname, count(*) AS learnt_techs
FROM developers dev
         INNER JOIN developer_technology dt ON dev.id = dt.developer_id
WHERE dt.technology_id != dev.main_technology_id
GROUP BY dev.id;

SELECT matricule, lastname, firstname, tech.name_en as main_tech_en
